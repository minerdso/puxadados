...
const { imageToWebp, videoToWebp, writeExifImg, writeExifVid } = require('./services/lib/exif')
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetchJson, await, sleep } = require('./services/lib/myfunc');
...
let prem2 = [
  ...
  `5511959778542@s.whatsapp.net`, //samira
 ...
 • Proibido qualquer tipo de divulgação e venda de serviços/produtos sem ser o nosso.
 • Proibido fotos, videos, GIFs e arquivos em geral.
 • Proibido flodd de comandos e/ou mensagens.
 • Proibido links em geral a não ser o do grupo em questão.

 O grupo foi feito para a divulgação do nosso serviço e proporcionar consultas grátis, o descumprimentos das regras resultarão em seu BANIMENTO.
 ━━━━━━━━━━━━━━━━━━
 - Para ver as consultas disponíveis digite: !menu

 - Para comprar algum plano chame algum adm`


//CONFIGURAÇÃO DA DATABASE
const { conectar_db } = require('./services/connect');
conectar_db();
const {
apikeypremium
} = require("./config.js");
...
try {
  ...
  low = require('./services/lib/lowdb')
}

const { Low, JSONFile } = low
const mongoDB = require('./services/lib/mongoDB')
...
async function startclient() {
    const client = clientConnect({
        ...
        browser: ['Scorpion Multi Device','Safari','1.0.0'],
        auth: state
    })
    ...
    client.ev.on('group-participants.update', async (anu) => {
        console.log(anu)
        number = anu.participants
        const isP = prem2.includes(number)
        if(isP){
            msgg = 'Premium detectado, aproveite'
        } else {
            msgg = 'Premium não detectado, compre um plano'
        };
      ...
      try {
        ...
        for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    client.sendMessage(anu.id, { image: { url: 'https://app.minerdso.com.br/UTIL/BOTPUXADA.jpg' }, contextInfo: { mentionedJid: [num] }, caption: `BOAS VINDAS 🔍\n\n• Seja bem-vindo(a) @${num.split("@")[0]}\n\n• ${msgg}\n\n━━━━━━━━━━━━━━━━━━${desccc}\n━━━━━━━━━━━━━━━━━━`})
            }
        }
        } catch (err) {
            console.log(err)
	...
	for (let i of kon) {
	    list.push({
	    	...
	    	vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await client.getName(i + '@s.whatsapp.net')}\nFN:${await client.getName(i + '@s.whatsapp.net')}\nitem1.TEL;waid=${i}:${i}\nitem1.X-ABLabel:Número\nitem2.EMAIL;type=INTERNET:kauanjardini@gmail.com\nitem2.X-ABLabel:Email\nitem3.ADR:;;Brasil;;;;\nitem4.X-ABLabel:Algum Lugar\nEND:VCARD`
	    })
	}
	client.sendMessage(jid, { contacts: { displayName: `${list.length} contatos`, contacts: list }, ...opts }, { quoted })
    }
